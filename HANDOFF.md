...
- **Hero sub-line + body width matched to the H1.** They were capped at `max-width:560px` while the H1 runs to 730px, so they wrapped a few words early. Both bumped to **`max-width:730px`** so all three align to the same right edge; the body dropped 3→2 lines.
