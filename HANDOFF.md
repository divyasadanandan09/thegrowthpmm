...
## Update 2026-07-03 — client edit pass (nav/footer/blog, justified copy, OG, hero, good-fit)

A 12-point client punch list. All on `main`-to-be, build clean, no console errors. Verified at 1440 desktop + 375 mobile via the dev server.

- **Hero H1** → **"The first Product Marketing leader you need, before you're ready to hire one."** (added "first"; lime `.marker` unchanged on "before you're ready to hire one"). Client wanted it on **2 lines** — the old 1.05/0.95 hero grid column (~596px) forced 3 lines, so the grid was rebalanced to **`1.5fr 0.8fr`** (gap 56→48) to widen the text column to ~746px, and H1 dropped 42→**38px** (`max-width:730px`). Now exactly 2 lines at desktop; diagram still renders at its 380px max. Mobile still 36px, 1-col.
- **Copy alignment → fully justified** (client-directed). Added `main p { text-align: justify; text-justify: inter-word; }` in `global.css`. Only multi-line paragraphs are affected. **Deliberately-centered promo paragraphs opt out via inline `text-align:center`**: index testimonials CTA subline, Growth-Audit intro, closing-band lime subheading + reassurance; faq closing subline; ai-page closing subline. (The audit-card "You'll walk away with" label already had inline center.) Footer sits outside `<main>`, so footer text stays left. **Tradeoff:** on the 375px mobile column, justify opens visible word-gaps on short lines — inherent to justify, left as-is per the explicit request; revisit with a `@media` left-align override if the gaps read badly.
- **Footer restructured.** Offer column: **dropped Proof** (now just Growth Audit / Services). Company column → **About me / Blog / FAQ** (renamed "About"→"About me", **removed the "Book free consultation" footer link** — not SEO-relevant, and **removed AI PMM** which now lives under Blog).
- **Blog created.** New **`/blog`** index (`src/pages/blog.astro`) — a `.blog-card` list; **AI PMM (`/ai-product-marketing`) is its first post**. Footer "Blog" → `/blog`. Future posts: add to the `posts` array at the top of `blog.astro`.
- **Nav pill de-cramped (was wrapping "Free Growth Audit"/"About me" into stacked words).** `#navbar` max-width 640→**720px**, gap 22→20, and **`#nav-links a { white-space: nowrap }`** so each link stays on one line. Items unchanged (Services / Free Growth Audit / About me).
- **Mobile menu trimmed to match desktop** — now just **Services / Free Growth Audit / About me + CTA** (dropped Proof / AI PMM / FAQ; those live in the footer). 
- **Mobile menu closes on scroll.** (Revised same session — first tried a `body.menu-open { overflow:hidden }` scroll-lock, but the client wanted the menu to *close* when the page scrolls, not freeze it.) The scroll handler now calls `closeMenu()` the moment `window.scrollY` moves while the menu is open (plus close on any menu-link click). No scroll-lock CSS.
- **Hero sub-line + body width matched to the H1.** They were capped at `max-width:560px` while the H1 runs to 730px, so they wrapped a few words early. First bumped to 730px, but the justified paragraphs then overshot the H1's *balanced* text (its widest line only reaches ~692px, not the full 730px box). Final: sub-line + body set to **`max-width:692px`** so their justified right edge sits flush with the headline's widest line (verified all three right edges = 692px via Range `getClientRects`). Body is 2 lines.
- **Good-fit cards → equal-height matched pair** (`/impeccable polish`, 2026-07-03). Switched `.fit-grid` from `align-items:start` to **`stretch`**, made `.fit-grid .compare-card` a flex column, and gave `.fit-note` `margin-top:auto` so the shorter white card stretches to the near-black card's height (both 343px now) with the note anchored to its base (~37px below the list). Pair gap 18→20px. Reads as a deliberate matched pair instead of two mismatched-height cards.
- **`/impeccable critique` (2026-07-03) — scored 33/40 "Good"** (trend 31 → 34 → 33; snapshot `.impeccable/critique/2026-07-03T04-14-19Z__src-pages-index-astro.md`). Detector was clean bar one warning (Inter body font, mitigated by the Bricolage/Caveat pairing). Two P2 fixes shipped from it (client-approved scope):
  - **Mobile menu reachable after scroll (P2, `/impeccable adapt`).** The nav pill (with the hamburger) goes `pointer-events:none` past 80px scroll, so on mobile the menu was unreachable once scrolled — only the corner CTA remained. Added **`#corner-menu-toggle`** (a 44px round hamburger) into `#nav-cta-corner`, shown only ≤900px; it toggles the same `#mobile-menu`. Both toggles keep `aria-expanded` in sync (shared `openState`/`closeMenu` in the nav `<script>`).
  - **Justify is now desktop/tablet-only (P2, `/impeccable typeset`).** `main p { text-align:justify }` opened word-gaps on the ~335px phone column, so **`@media (max-width:600px) { main p { text-align:left } }`** left-aligns body copy on phones while keeping the justified block look ≥600px. (Centered promo p's still win via inline `text-align:center`.)
  - **Still open from the critique (client to provide / decide):** the "Varun" placeholder testimonial (P2, real named quote needed); the `DIAGNOSTIC` Google Form URL + `astro.config.mjs` `site:` domain (P3 placeholders); `/blog` has one post (P3, thin); no custom 404 (P3).
- **Email → `divya@thegrowthpmm.com`** (was thegrowthpmm@gmail.com) in the footer mailto **and** the `ProfessionalService` schema `email` in `Layout.astro`. CLAUDE.md's contact-email line updated too.
- **OG / meta reworked.** `Layout.astro` default `title` → **"The Growth PMM | Fractional Head of PMM"**; default `description` → the client's paragraph ("The first Product Marketing leader you bring in before you hire a full-time one. I help post-PMF startups build the positioning, messaging, and GTM…"). **`public/og-image.png` regenerated** (1200×630) on-brand: cream/grid-canvas bg + aurora, the lime funnel **badge + "The Growth PMM"** wordmark, a big Arial-Black headline with a **lime marker** highlight on "before you hire a full-time one.", muted subtext, a near-black "Fractional Head of PMM" pill, and the lime bottom accent bar. Generator script: `scratchpad/make_og.py` (PIL; fonts = Arial Black/Bold/Regular; now committed — re-run it if the OG copy changes). The old navy-funnel "Find what's capping your growth" OG is replaced.

**Update 2026-07-03 (later) — OG image layout fix (`/impeccable layout`).** The `thegrowthpmm.com` URL line that sat bottom-left (opposite the "Fractional Head of PMM" pill) was dropped — redundant since the sharing platform already shows the link's own domain next to the card. Moved the pill from bottom-right to bottom-left, under the badge/headline column, so the card reads as one clean left rail (badge → headline → subtext → pill) instead of two competing bottom anchors. `scratchpad/make_og.py` updated and regenerated; committed.
- **"Are we a good fit?" polished** (`/impeccable layout`). The compare grid kept its bio-column-aligned ~920px width (the right gutter is intentional — the About photo occupies that zone above), but the cards now **size to their own content (`align-items:start`)** instead of force-stretching, so the shorter "Probably not, if" card no longer strands dead space. Added a hairline-separated **`.fit-note`** to that card ("Not sure where you land? A 20-minute call will make it clear, either way.") — balances the heights (343px vs 326px now) and doubles as a soft, on-brand call nudge. New classes: `.fit-head`, `.fit-grid`, `.fit-note` in `global.css`.

## Update 2026-07-04 — `/impeccable critique` #4 + code-fix pass

Critique scored **31/40 "Good"** (trend 31 → 34 → 33 → 31; snapshot `.impeccable/critique/2026-07-04T17-19-44Z__src-pages-index-astro.md`). The dip was one concrete regression: the 2026-07-03 `main p { text-align:justify }` directive opening word-rivers in **narrow desktop columns** (the phone-only left-align fix didn't cover column width). Fixes shipped this session (all code; build clean, no console errors):

- **Justify scoped to wide prose (P1).** Added `.module p, .process-step p, .proof-strip p, .proof-bottom p, .recognition-band p { text-align:left }` in `global.css`. Worst offender was the recognition band's "Top 100 Product Marketing Influencer globally" headline rendering with big gaps. Wide single-column paragraphs (hero, section leads) stay justified per the client's 2026-07-03 direction.
- **Hero diagram chips legible (P2).** Product/Sales/Marketing outline chips `stroke-opacity` 0.4 → 0.7, width 1.2 → 1.3 (they were dissolving into the grid canvas at 1440px).
- **Marker punctuation (P3).** Trailing punctuation moved *inside* the `.marker` spans in all four spots (hero H1, problem lead, proof H2, About bio) — the 0.18em marker padding was visually detaching the period ("hire one .").
- **Consistency sweep (P2):** "on incremental spend" → "On incremental spend" (stat sublabel casing); "for traveltech startup" → "for a traveltech startup"; recognition trailing-period inconsistency fixed ("at Fi Money." → no period, matching "at Practo"); footer tagline realigned to the current positioning: "Built to do one job: **give your growth a Product Marketing owner.**" (was the retired "find what's capping your growth").
- **`.report-check` chips → lime** (`--navy` fill + `--ink-deep` check) on the dark Growth Audit card; they were pre-rebrand mint/green (#e6f5ef/#007a5a) and the in-page detector flagged them. The compare-card green ✓ / red ✗ pair is deliberately unchanged (fit/not-fit semantics).
- **Custom 404 added** (`src/pages/404.astro`): on-brand grid-canvas/aurora page, handwritten "well, this is a dead end" kicker, links home + booking. Builds to `/404.html` (Cloudflare serves it automatically for static 404s).
- **Mobile footer clearance:** `footer { padding-bottom:72px }` in the ≤900px block so the bottom-docked corner CTA no longer covers the copyright line.

**Still open from this critique (not code):** the "Varun" anonymized lead testimonial (P2, top trust lever — client to supply a real named quote); blog thinness (2 posts) vs the "leading voice" ambition; detector's mild thin-border+wide-shadow tells (navbar pill, featured module, `.compare-card--me`) deliberately left — they're part of the approved look.

## Update 2026-07-05 — client copy pass + hero reword + Before/After redesign + corner-CTA move

A client punch list (copy + a few design asks). All on `main`, build clean, no console errors, verified at 1280px desktop via the dev server + DOM measurement.

**Hero reworked (client-supplied copy).**
- Tag → **"Top 100 PMM, 2025"** (was the scarcity tag "Only 3 clients per month"). This **reverses** the 2026-07-02 scarcity-tag decision; the three-clients-a-month scarcity is now surfaced nowhere in the hero. The tag duplicates the award already in the Recognition band lower down — kept intentionally as a top-of-page credibility signal (flagged to client).
- H1 → outcome-led **"Product Marketing that turns early traction into scalable growth."** (lime `.marker` on "scalable growth."). The old positioning headline moved down into the **bold sub-head: "The first Product Marketing leader you'll need before you're ready to hire one."**
- Body → "I partner with post-product-market-fit startups to clarify positioning, sharpen messaging, and execute GTM strategies that grow adoption and revenue, and prepare you for your next stage of growth." (one stray comma from the client draft cleaned up for grammar).
- **Hero sub-head + body are now `text-align:left` (inline)** — they opt out of the global `main p { text-align:justify }` rule. The client flagged the justify was opening word-gaps and pushing the paragraphs' right edge past the H1's ragged edge; left-aligning sits them flush to the headline's left border, ragged-right like the H1. (Element types unchanged: `h1` + bold `p` sub-head + body `p`, so the tuned hero styling is preserved.)

**Services.** Dropped the featured card's trailing sentence "The three foundations below are how we get there." (it undercounted the four services / conflicted with the "Four foundations" subhead). The featured "A repeatable growth engine" desc now ends at "…so growth becomes intentional, not reactive." Subhead stays "Four foundations that turn early traction into growth you can repeat."

**Problem→transformation copy.** "…those jobs **get too interconnected** to stay founder-led" → "…those jobs **become too complex** to stay founder-led." Before/After row "Clear **PMM** ownership" → "Clear **Product Marketing** ownership."

**Before/After redesigned (`.ba-table`, `/impeccable`-style elevation).** Was a flat 2-col struck-through/pale-lime table. Now a **"resolved ledger"**: white card, `box-shadow:0 18px 44px rgba(22,24,15,0.10)`, 20px radius; each **Before** row gets a muted **✗** chip + struck text, each **After** row a lime **✓** chip + bold ink (reuses the `.compare-card` ✗/✓ vocabulary from "Are we a good fit?"), rows lift on hover. Row text wrapped in `<span>` so the line-through hits only the text, not the ✗ chip. *(Alternatives considered + rejected: two contrasting panels — breaks the 1:1 before→after mapping; inline strike→lime-marker list — busy at 5 rows; 2×5 card grid — reintroduces the card-monotony AI-slop tell.)*

**Same-session follow-up: ledger made playful (`/impeccable polish`).** Client saw the resolved ledger live and called it dry, asked for more fun using design elements, without an AI-slop relapse (no side-stripe, no card grid, no emoji). Reused the site's own "rotated note" vocabulary rather than inventing a new one: a **`.sticky-tag.gold` "Resolved" stamp** now overlaps the card's top-right corner, rotated `-6deg`, popping to `-3deg`/`scale(1.05)` on hover — it literally stamps the "resolved" metaphor already in the name, and reuses the exact tag component Services/Recognition already use. Each row's ✗/✓ chip got an **alternating rubber-stamp tilt** (`--chip-tilt: 8deg` odd rows / `-8deg` even rows) that straightens + pops to `scale(1.15)` on that row's own hover, directly echoing how the How-it-works `.process-num` badge straightens on card hover (2026-07-05, same day) — same easing curve (`cubic-bezier(0.22,1,0.36,1)`, 0.32s), so the two "make it fun" passes feel like one system, not two. Structural change: `.ba-table`'s own margin moved onto a new wrapping `.ba-wrap`, since the stamp needs `position:absolute` off a relative parent that isn't clipped by the table's `overflow:hidden` (needed for the rounded-corner header background). Mobile: stamp shrinks (`font-size:11px`, tighter offset) at the same `900px` breakpoint the ledger's own padding already tightens at. *(Considered and skipped: tilting whole rows like `.process-step` — breaks the ledger's straight-line readability, which is the point of a table; a scroll-triggered reveal per row — the site already avoids uniform scroll-reflex animation as a tell, kept hover-only instead.)* Verified with `npm run build` (clean, generates `.ba-stamp`/`.ba-wrap` in output) and by reading the rendered `dist/index.html`; **could not get a live-browser check this session** — another chat session already holds the project's dev server on port 4321 and this session's preview tools can't reach a second instance. If you're reading this after the fact, sanity-check the stamp position and chip-tilt easing on a live reload before calling this fully verified.

**About.** H2 "Why I do this." → **"Why work with me."** and the **"twelve years in, one lesson" handwritten eyebrow was dropped** (section now opens straight on the H2; the "let's be honest" eyebrow on the good-fit block is untouched).

**"Are we a good fit?" boxes leaned out.** `.fit-grid` dropped its `max-width:900px` cap → **full section width** (~586px cards); padding `28px 24px`→`22px 26px`, list gap `14px`→`11px`, head margin + note padding tightened. Cards went from ~343px to **~250px equal height** — slim horizontal panels instead of tall blocks.

**Floating corner CTA moved to bottom-right on desktop (client).** `#nav-cta-corner` was `top:16px;right:16px` → **`bottom:24px;right:24px`** (hidden-state transform flipped `translateY(-12px)`→`translateY(12px)` so it slides up from the bottom). Rationale: bottom-right is the always-in-view, easier-to-click convention and now matches mobile (which already docked to the bottom). Caught + fixed an overlap: at page end it covered the footer tagline, so added a base **`footer { padding-bottom:64px }`** (the ≤900px block already had 72px).

## Update 2026-07-05 (later) — "How it works" made playful (`/impeccable polish`)

Client feedback: the 3-step "How it works" section (previously three identical flat white cards, each with a plain uppercase "Step N" line, no icon, no color, no connective visual) read as boring, and asked to make it "more fun" using design elements, staying on-brand. Ran through `/impeccable` (register: brand, existing design system).

**Shipped (`index.astro` `#how-it-works` + `.process-*` in `global.css`):**
- Added a **handwritten Caveat kicker** ("three moves, one owner") above the H2, matching the kicker pattern used elsewhere (Services' "the work itself", About's "let's be honest") — this section previously had none.
- Each `.process-step` card now carries a **subtle scrapbook tilt** (`--tilt`, alternating roughly ±1–1.5deg, set inline per card) that **straightens and lifts on hover** (`translateY(-6px)`, border shifts to lime, deeper shadow).
- The plain "Step N" uppercase text line is **gone**, replaced by a small **rotated lime `.process-num` circle badge** (Bricolage Grotesque numeral, ink-deep on lime) overlapping the card's top-left corner — reuses the `.sticky-tag` rotated-note vocabulary already established (Services' "the outcome we build toward" tag, the Recognition band tags). The badge has its own independent tilt that also straightens on hover.
- A **monoline SVG arrow connector** (`.process-connector`, same icon spec as Services/Recognition/AI-page icons: viewBox 0 0 24 24, stroke-width 1.8, `aria-hidden`) sits between each pair of cards on desktop, echoing the hero diagram's connector-line language and reinforcing the step-by-step flow. Desktop grid changed from `repeat(3, 1fr)` to `1fr auto 1fr auto 1fr` to fit the connector columns.
- **Mobile (`<900px`):** connectors are hidden, tilt is reset to `none` (hover still gets a straight lift), and the grid stacks to one column, same as before — the numbered badges alone carry the sequence top-to-bottom.
- **Reversal noted:** the `.process-num` circle badge existed pre-2026-07-01 and was deliberately removed then (alongside "Week 1-2" timing labels) to declutter the section. Reintroducing it now, in a new rotated/overlapping form, is intentional given the explicit "make it fun" ask — it's a different visual treatment (tilted lime badge, not a plain centered circle), not an undo of the earlier decluttering rationale.
- No new colors introduced — reuses lime (`--navy`), `--ink`, `--ink-deep`, `--ink-mute`, Bricolage Grotesque, and Caveat, all already established. No card-monotony regression: the three cards are still visually near-identical in shape (a legitimate real 3-step sequence, unlike the old "4 identical services cards" AI-slop tell), but now read as connected + textured rather than a flat repeated grid.
- Verified on the live dev server: DOM/computed-style checks (grid-template-columns collapses correctly at both breakpoints, `.process-connector` display:none + tilt reset on mobile, badge/card transforms compute as expected), full-page screenshot at a tall viewport (works around the known deep-scroll blank-screenshot issue), zero console errors. Not yet committed.

## Update 2026-07-05 (even later) — mobile-only length trims (client-directed, desktop untouched)

Client asked to shorten several sections **on mobile only** — the homepage was "too long" on phone. Every change below is scoped inside the existing `@media (max-width: 900px)` block in `global.css` (or a class hidden only there); **desktop is byte-for-byte unaffected**, verified at 1440px after the change.

- **Services** — `.module-desc` (the paragraph under each card title) is hidden on mobile. Titles + icons only.
- **How it works** — `.process-desc` (the paragraph under each step title) is hidden on mobile. Numbered badges + titles only.
- **Impact / Proof stats** — each of the 3 `.proof-stat` cards ($11M+ / 7.35x / 25%) got its explainer paragraph turned into a **mobile-only accordion**: a hidden checkbox (`.proof-toggle`, the classic CSS-only checkbox-hack pattern) + a `<label>` wrapping the existing bold title line + a chevron SVG, with `.proof-desc` collapsed by default and revealed via `:checked ~ .proof-desc`. The **first stat's checkbox ships `checked`** so its explainer is open by default; the other two start collapsed. On desktop the checkbox stays permanently hidden (`position:absolute;opacity:0`), the label renders as a plain block (identical to the old bare `<p>` it replaced), the chevron stays `display:none`, and `.proof-desc` is untouched outside the mobile query — so desktop markup changed (new checkbox/label wrapper) but desktop rendering did not.
- **"Are we a good fit?" fit table** — `.fit-head` (the "let's be honest" kicker + H3) and `.fit-grid` (the Yes-if/Probably-not-if cards) are both `display:none` on mobile. Dropped entirely, not just shortened; the About section now flows straight from the bio into the Growth Audit section on phone.
...
2. **Hero** — call-led. *(Reworded 2026-07-05 — see that update above.)*
   - Tag: **"Top 100 PMM, 2025"** (`.sticky-tag purple`, lime fill; was the scarcity tag "Only 3 clients per month").
   - H1: **"Product Marketing that turns early traction into scalable growth."** (lime `.marker` on "scalable growth.").
   - Bold sub-head **"The first Product Marketing leader you'll need before you're ready to hire one."** + body paragraph ("I partner with post-product-market-fit startups to clarify positioning, sharpen messaging, and execute GTM strategies that grow adoption and revenue, and prepare you for your next stage of growth."). **Both are `text-align:left` (inline)** — opting out of the global justify so they sit ragged-right flush to the H1's left edge.
...
4. **Problem → transformation (merged Sections 2+3, `--cream` bg)** — H2 "Growth didn't get harder. Your company got more complex." (one line at desktop) → lead paragraph (lime `.marker` on "nobody owns the story that connects them") → "Here's the shift once Product Marketing has an owner." → the **`.ba-table` Before → After** table (redesigned 2026-07-05 into a "resolved ledger": white card + soft shadow, muted ✗ chip on each struck Before row / lime ✓ chip on each bold After row, hover lift).
...
6. **How it works (`#how-it-works`)** — handwritten kicker "three moves, one owner" (added 2026-07-05), H2 "How we'd actually work." Three steps (Understand your growth constraints / Build the Product Marketing foundation / Enable your team to scale), each a slightly tilted `.process-step` card with a rotated lime numbered badge (replaces the old plain "STEP 1/2/3" text) and a monoline arrow connector between cards on desktop (see the 2026-07-05 "made playful" update below). Inline "Book free consultation" CTA.
...
9. **About + why me (`#about`)** — H2 **"Why work with me."** (the "twelve years in, one lesson" eyebrow was dropped 2026-07-05; section opens straight on the H2). Story bio (Fi Money/Practo/MakeMyTrip/Raymond, the "gap Product Marketing fills" thesis). Circular photo **medallion** on the right. **Note: the old "I take only three clients a month" marker line was dropped** (scarcity now surfaced nowhere in the hero either). Closes with the **"Are we a good fit?"** block: "let's be honest" kicker, H3, full-width leaner `.fit-grid` ("Yes, if" ✓ 5 items / "Probably not, if" ✗ 4 items; extended + leaned out 2026-07-05).
...
- [x] Domain swapped — `astro.config.mjs` `site:` is `https://www.thegrowthpmm.com` (2026-07-03); canonical/OG/schema all resolve to www. `public/og-image.png` exists.
- [x] **Apex → www redirect** — set up 2026-07-04 as a Cloudflare zone-level Redirect Rule (Rules → Redirect Rules → "Redirect from root to WWW" template): wildcard match `https://thegrowthpmm.com/*` → `https://www.thegrowthpmm.com/${1}`, 301, preserve query string on. Verified: `curl -sI https://thegrowthpmm.com/` and a `?utm_source=test` URL both return `location: https://www.thegrowthpmm.com/...` with the query string intact.
...
- [ ] Unused CSS to prune: `.service-row` / `.service-num` / `.service-icon`, `.audit-step`, and now **`.eyebrow`** (uppercase kicker, unused everywhere since 2026-07-02) and **`.ownership-row`** (fragments removed in the section merge). Note `.module--featured` is back IN USE (Services featured card), `.channel-grid`/`.channel-card` are IN USE on `/ai-product-marketing`, and **`.process-num` is back IN USE** (2026-07-05, How it works numbered badges) — don't prune those.
